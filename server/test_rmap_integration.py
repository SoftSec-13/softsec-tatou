...
def create_simple_pdf():
    ...
    page.insert_text((50, 150), "This PDF will be watermarked with an RMAP session secret.", fontsize=12)
    
    pdf_path = "/tmp/rmap_integration_test.pdf"
    doc.save(pdf_path)
    doc.close()
    return pdf_path
...
def watermark_pdf_with_rmap(pdf_path, session_secret):
    ...
    try:
        ...
        if read_secret == session_secret:
            ...
            print(f"   ✓ Read secret matches RMAP session secret")
...
def main():
    ...
    print(f"   • RMAP authentication: ✓")
    print(f"   • PDF watermarking: ✓")
    print(f"   • Watermark verification: ✓")
    print(f"   • Session secret preserved in PDF: ✓")
