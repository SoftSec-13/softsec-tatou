#!/usr/bin/env python3
"""Fuzz input validation (SQL injection, file upload, path traversal)."""
...
def fuzz_sql_injection(fdp: atheris.FuzzedDataProvider):
    ...
    if fdp.ConsumeBool():
        pattern = SQL_PATTERNS[fdp.ConsumeIntInRange(0, len(SQL_PATTERNS) - 1)]
        email = pattern + email
    ...
    try:
        with app.test_client() as client:
            resp = client.post(
                "/api/create-user",
                json=payload,
                headers={"Authorization": common.make_auth_header()},
            )
...
def fuzz_file_upload(fdp: atheris.FuzzedDataProvider):
    ...
    if fdp.ConsumeBool():
        prefix = SQL_PATTERNS[fdp.ConsumeIntInRange(0, len(SQL_PATTERNS) - 1)]
    ...
    try:
        ...
        common.cleanup_storage()


def fuzz_inputs(data: bytes):
...
def main():
    atheris.Setup(sys.argv, fuzz_inputs)
    atheris.Fuzz()
