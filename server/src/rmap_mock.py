...
class MockIdentityManager:
    ...
    known_identities: Dict[str, bool] = field(default_factory=lambda: {
        "Jean": True,
        "Alice": True, 
        "Bob": True
    })
    ...
    def encrypt_for_identity(self, identity: str, payload_obj: dict) -> str:
        ...
        try:
            ...
            raise EncryptionError(f"Mock encryption failed for identity '{identity}': {exc}") from exc
...
class MockRMAP:
    ...
    This simulates the same interface as the real RMAP class but without PGP dependencies.
    """
    ...
    def handle_message1(self, incoming: dict) -> dict:
        ...
        except (ValidationError, DecryptionError, EncryptionError, UnknownIdentityError) as exc:
