...
def load_group13_private_key():
    ...
    if key.is_protected:
        ...
        for attempt in range(max_attempts):
            password = getpass.getpass(f"Enter password for Group 13 private key (attempt {attempt + 1}/{max_attempts}): ")
            try:
                ...
                if attempt == max_attempts - 1:
                    raise Exception("Failed to unlock private key after maximum attempts")
    else:
...
def create_rmap_message1(server_pub_key, nonce_client=12345678901234567890, identity="Group_13"):
...
def decrypt_server_response(encrypted_payload, group13_private_key):
    ...
    try:
        ...
        if group13_private_key.is_protected and hasattr(group13_private_key, '_stored_password'):
...
def test_dynamic_rmap_flow():
    ...
    try:
        ...
        response1 = requests.post(f"{SERVER_URL}/rmap-initiate", json=msg1_payload)
        ...
        print(f"Server response payload length: {len(response1_data.get('payload', ''))}")
        ...
        response2 = requests.post(f"{SERVER_URL}/rmap-get-link", json=msg2_payload)
        ...
        if "result" in response2_data:
            ...
            print(f"✓ RMAP flow completed successfully!")
